...
public class MarsBot implements Observer, MarsTool {
    ...
    private static final int TRACK_POINT_COUNT = 256;  // TBD Hardcoded. Array contains start-end points for segments in track.
    private final Point[] trackPoints = new Point[TRACK_POINT_COUNT];
        ...
        public void run() {
            ...
            while (true) {
                if (MarsBotMoving) {
                    ...
                    try {
                        Application.memory.setWord(ADDR_WHEREAREWEX, (int) MarsBotXPosition);
                        Application.memory.setWord(ADDR_WHEREAREWEY, (int) MarsBotYPosition);
                    }
                    ...
                    trackPoints[trackIndex] = new Point((int) MarsBotXPosition, (int) MarsBotYPosition);
                }
    ...
    private class MarsBotDisplay extends JPanel {
        ...
        public void paintComponent(Graphics g) {
            ...
            for (int i = 1; i <= trackIndex; i += 2) { // Index grows by two (begin-end pair)
                try {
                    g2.drawLine((int) trackPoints[i - 1].getX(), (int) trackPoints[i - 1].getY(), (int) trackPoints[i].getX(), (int) trackPoints[i].getY());
                }
    ...
    public void action() {
        ...
        try {
            Application.memory.addObserver(this, 0xffff8000, 0xffff8060);
        }
